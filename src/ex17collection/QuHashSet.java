...
//		System.out.println("hashCode() 호출됨"+ nameHCode);		//확인용 출력문장
		//멤버변수가 2개 이상이면 고유한 주소값을 더해서 반환하면 된다.
		return nameHCode + heroHCode + weaponHCode;
	}
	@Override
	public boolean equals(Object obj) {
		System.out.println("eqauls() 호출됨");		//확인용 출력문장
		//매개변수로 전달된 객체를 Avengers타입으로 다운캐스팅한다.
		Avengers avengers = (Avengers)obj;
		//각각의 멤버변수가 가진 값에대한 비교를 진행한다.
	...
	public static void main(String[] args) {
		...
		while(itr.hasNext()) {
			...
			if(searchName.equals(avn.name)) {
				...
				isFind = true;	//정보를 찾았다면 true로 변경
			}
		}
		if(isFind==false) System.out.println("해당 영웅은 없어요ㅠㅠ");
	}
}
