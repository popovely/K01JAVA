...
public class R03Method
{
	...
	public static void guguPrint() {
		...
		for(int dan=2; dan<=9; dan++) {
			for(int su=1; su<=9; su++) {
				//dan이 2로 고정된 상태에서 su는 1~9까지 증가한다.
				System.out.printf("%2d * %2d = %-4d", dan, su, dan*su);
			}
			//2단이 출력되면 줄을 바꾸고 dan이 1 증가한다.
			System.out.println();
		}
	}
	
	//메소드 타입2 : 주어진 범위를 출력하는 구구단
	public static void guguRangeShow(int n1, int n2) {
		//매개변수로 전달된 n1단에서 n2단까지 출력
	...
	public static double sungjukResult() {
		...
		//실수(double)의 결과를 얻기위해 3.0으로 나눈다.
		double avg = (kor+eng+math) / 3.0;
		return avg;
	}
	
	//메소드 타입4 : 주어진 값까지의 합을 구해서 반환하는 메소드
	...
	public static void main(String[] args)
	{
		...
		/*
		[시나리오]
		sungjukResult() 메소드를 호출하면 국어,영어,수학 점수를 입력받은후
		평균값을 반환하도록 구현하시오.
		
		 - 매개변수X, 반환값O
		 */
		System.out.println("매개변수X, 반환값O");
		//반환값이 있는 메소드는 좌측항이 있거나 print()내에서 호출한다.
		System.out.print("평균값은? "+ sungjukResult());
		
		/*
		[시나리오]
		정수 하나를 매개변수로 전달하면 1부터 해당 매개변수 까지의 합을 구하여
		결과값을 반환하는 메소드를 작성하시오.
		...
		/*
		[연습문제]
		다음과 같은 출력결과를 보이는 프로그램을 메소드로 구현하시오.
		메소드명 : upgradeGugu()
		실행결과] 
		---------- java ----------
		2x1=2 2x2=4
