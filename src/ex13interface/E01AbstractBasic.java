...
/*
위 클래스를 상속하면 부모가 가진 추상메소드를 그대로 상속받게 되므로
해당 클래스도 abstract로 선언해야 한다.
이 경우 해당 클래스로도 객체를 생성할 수 없게된다.
만약 객체를 생성해야 한다면 추상메소드를 자식이 오버라이딩하면 된다.
 */
...
//		AbsClass1 absClass1 = new AbsClass1();	->에러발생.
		...
		//추상클래스를 상속받은 하위클래스는 객체생성이 가능하다.
		AbsClassChild1 absClassChild1 = new AbsClassChild1();
		
		/*
		추상클래스로 인스턴스 생성은 할수없지만 참조변수로 사용하는것은 가능하다.
		 */
		...
		/*
		추상클래스 내에 선언된 정적멤버메소드는 오버라이딩의 대상이 아니므로
		static의 기본규칙을 따른다.
		;클래스명으로 접근하면 된다.
		 */
