...
-객체생성은 할 수 없으나 그외 참조변수, 상속 등의 모든 기능을 할 수 있다.

추상메소드
-오버라이딩의 목적으로 제작된다.
-메소드의 실행부가 없기때문에 중괄호 {} 대신에 ;을 붙여준다.
 */
...
class PersonalInfoDTO {
	...
	/*
	getter/setter메소드
	 : DTO객체에서 private으로 선언된 멤버변수의 값을
	   설정하거나 반환할때 호출하는 메소드
	 */
...
/*
제공된 추상클래스를 상속하여
사람의 정보를 저장하는 기능과 검색의 기능을 구현한 클래스이다.
 */
class PersonalNumberStorageExt extends PersonalNumberStorageAbs {
	...
	/*
	상속을 통해 추상메소드를 포함하게 되었으므로
	반드시 아래와 같이 오버라이딩 해야한다.
	필수사항이며 하지 않을경우 에러가 발생한다.
	 */
	...
	public String searchPersonalInfo(String juminNum) {
		for(int i=0; i<numOfPerInfo; i++) {
			...
			이럴때 getter메소드를 통해 접근해서 값을 얻어올 수 있다.
			 */
...
public class E02AbstractToInterface1 {
	...
	public static void main(String[] args) {
		...
		System.out.println(storage.searchPersonalInfo("901234-3333333"));
	}
}
