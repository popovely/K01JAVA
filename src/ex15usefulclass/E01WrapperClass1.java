...
public class E01WrapperClass1 {
	...
	public static void main(String[] args) {
		...
		다시 언박싱을 위해 intValue()와 같은 메소드를 호출해야 한다.
		 */
		Integer n10Obj = new Integer(10);	//현재는 이와같은 생성자는
		Integer n20Obj = new Integer(num2);	//디프리케이트(deprecated) 되었다.
		...
		오토박싱 혹은 오토언박싱이 지원되므로
		기본자료형 변수를 선언하는것과 동일하게 기술할수 있다.
		 */
		Integer numObj1 = 100;
		Integer numObj2 = 200;
		int num3 = numObj1;
		...
		/*
		Wrapper클래스를 통해 진법변환을 하거나
		해당 자료형의 최소/최대값을 알 수 있다.
		 */
		...
		- toString()
		 : 인스턴스 변수를 출력할때 참조값이 아니라
		   실제 저장된 값을 문자열로 반환해주는 메소드
		- hashCode()
		 : 객체를 구별하기 위해
		   참조값을 고유한 정수값으로 반환해주는 메소드
		 */
