...
class FruitSeller {
	//멤버변수
	int numOfApple = 100;	//판매자가 보유한 사과개수
	int myMoney = 0;	//판매수익
	...
	2. 금액/사과의 단가=판매될 사과의 개수를 구한다. (5000/1000=5개)
...
//구매자를 추상화한 클래스
class FruitBuyer {
	int myMoney = 5000;	//구매자가 보유한 금액
	int numOfApple = 0;	//구매자가 구매한 사과의 개수
	
	/*
	구매자가 판매자에게 사과를 구매하는 행위를 표현한 멤버메소드
	1.판매자에게 금액을 지불한다.
	--> 여기에서 seller객체의 saleApple()메소드가 호출된다.
	2.판매자가 금액에 해당하는 사과의 개수를 반환하면,
	  해당 개수만큼 보유한 사과가 증가된다.
