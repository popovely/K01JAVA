...
/*
[시나리오]
여러개의 숫자를 입력받은후 최대값을 구하는 메소드를 정의하시오.
...
public class E05MethodType04_2 {
	...
	//main에서 전달한 인자 4는 매개변수 numberCnt가 받아서 사용한다.
	static int returnMaxNumber(int numberCnt) {
		...
		for(int i=1; i<=numberCnt; i++) {
			...
			if(i==1) {
				//입력받은 정수 중 첫번째 값을 기준값으로 설정한다.
				maxVal = inputNum;	//첫번째 값을 일단 최대값으로 지정한다.
			}
			else {
				//두번째 입력부터는 기준값인 maxVal과 입력값을 비교한다.
				if(maxVal<inputNum) {
					//만약 maxVal보다 입력값이 크면 입력값으로 교체한다.
					maxVal = inputNum;
				}
				//maxVal이 더 크다면 기존값을 유지하면 되므로 else{}는 필요없다.
			}
		}
		return maxVal;
	}
