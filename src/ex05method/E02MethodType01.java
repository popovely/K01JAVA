...
Method(메소드, 함수)
 ...
 즉 어떤 하나의 업무를 처리하는 모듈(부속품)이라 정의할수 있다.
 	...
 	규칙
 	- 메소드는 반드시 class 안에 정의해야한다.
 	- 메소드 안에 또 다른 메소드를 정의할 수 없다.
 	- 반환타입이 없다면 반드시 void를 명시해야 한다.
 	- 이름의 규칙은 변수명과 동일하다.	(Ex: addCalculate())
 
 Java에서의 NamingRule(명명규칙)
 	- 클래스명
 	...
 	- 메소드명, 변수명
 	 ...
 	 Ex)	simpleFunc() : 메소드명
 	  		simpleFunc	 : 변수명
 	- 상수명
 	 : 전체를 대문자로, 연결은 _(언더바)로 표현
 	 Ex)	SIMPLE_FUNC
 	- 패키지명
 	 : 전체를 소문자로, 연결은 .(점)으로 표현
 	 Ex) simple.func
 */
public class E02MethodType01 {
	...
	/*
	동일한 이름의 메소드를 정의하였으나
	매개변수의 개수로 호출할 대상을 구분할 수 있으므로
	다른 함수라 생각하면 된다.
	이와같은 문법을 "메소드 오버로딩"이라 한다.
	 */
	public static void noParamNoReturn2(int a) {
	}
	
	public static void noParamNoReturn3() {
		int returnValue = 89;
...
//		return; ->에러발생됨
		...
		따라서 return은 반드시 조건문과 함께 사용되어야 한다.
		단, 마지막 라인에 있을때는 상관없음.
		 */
