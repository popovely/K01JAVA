...
public class E02ArrayAndMethod
{
	...
	{
		//배열을 선언과 동시에 초기화. 요소의 개수를 통해 크기가 결정됨
		int[] arr = {1, 2, 3, 4, 5};
		//배열변수 선언
		int[] ref;
	...
	/*
	매개변수로 전달된 참조값을 ar이 받게되므로
	결국 동일한 배열을 참조하게 된다.
	즉 main() 메소드에서 생성한 배열과 동일한 배열을 참조하게 되는것이다.
	 */
	static int[] addAllArray(int[] ar, int addVal) {
		for(int i=0; i<ar.length; i++) {
			...
			/*
			ar[i] = ar[i] + addVal;
			위 복합대입 연산자를 풀어서 쓰면 이렇게 됨.
			즉 배열의 i번째 요소에 addVal을 더한후 다시 대입한다.
			여기서는 7씩 증가하는 연산이 수행된다.
			 */
		}
		return ar;
	}
