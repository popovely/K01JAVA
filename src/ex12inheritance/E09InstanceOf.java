...
public class E09InstanceOf
{
	...
	- Box타입으로 인자를 받으면 자동으로 업캐스팅이 되기 때문에
	  자식클래스의 멤버메소드를 호출할 수 없게된다.
	- 이때 자식의 멤버메소드를 호출하기 위해서는
	  다운캐스팅이 필요하다.
	- 따라서 전달된 인자를 각각의 타입으로 형변환이 가능한지
	  instanceof연산자로 판단한 후 다운캐스팅을 진행해야
	  각 클래스의 멤버메소드를 호출할 수 있다.
	 */
	...
	public static void main(String[] args)
	{
		...
		//각 타입의 인자를 부모타입인 Box로 한번에 받을 수 있다.
		wrapBox(box1);
		wrapBox(box2);
		wrapBox(box3);
		
		String str = new String("나는누구?");
//		wrapBox(str);	->String객체와는 상속관계가 없으므로 호출불가능
	}
}
