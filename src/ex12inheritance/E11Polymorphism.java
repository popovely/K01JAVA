...
-같은 타입의 참조변수로 객체 참조
1. 객체 전체를 접근할 수 있다. ;클래스의 일반적인 규칙이 적용된다.
...
//		parent1.childMember = 1;	->에러발생. 부모타입으로 자식멤버에 접근불가
		parent1.parentMethod();		//오버라이딩한 메소드이므로 자식쪽이 호출된다.
		
		Parent parent2 = new Child();
		parent2.parentMember = 1;
		parent2.parentMethod();
		
		//부모가 자식의 멤버에 접근하는 방법1 : 다운캐스팅 후 접근
		((Child)parent2).childMember = 1;
		((Child)parent2).childMethod();
		((Child)parent2).parentMethod();
		
		//부모가 자식의 멤버에 접근하는 방법2 : 자식타입의 참조변수에 할당한 후 접근
		...
		/*
		Java에서 생성한 모든 클래스는 직/간접적으로 Object클래스를 상속받는다.
		따라서 모든 객체는 Object의 참조변수로 참조 가능하다.
		또한 Object클래스에 정의된 모든 메소드를 별도의 선언없이 사용할 수 있다.
		 */
