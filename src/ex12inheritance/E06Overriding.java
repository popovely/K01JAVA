...
오버라이딩(Overriding)
	...
	멤버메소드와 완전히 동일한 모양으로 재정의 하는것을 말한다.
	오버로딩과 다른점은 매개변수의 갯수, 반환타입 등이 완전히
	똑같은 형태로 재정의 한다는것이다.
	하위클래스에서 오버라이딩한 메소드는 상위클래스의 메소드를 가리게되므로
	항상 최하위 클래스에서 오버라이딩한 메소드가 호출된다.
 */
...
//일반적인 스피커의 기능 + 중저음이 보강된 스피커를 추상화
class BaseSpeaker extends Speaker {
	...
	@Override
		: '어노테이션'이라 부른다.
		  오버라이딩된 메소드에 추가하여 컴파일러에게 알려주는 역할을 한다.
	 */
	@Override
	public void showState() {
		/*
...
public class E06Overriding {
	public static void main(String[] args) {
		...
		baseSpeaker.showState(); /*
		 	자식 객체로 호출하였으므로 부모쪽의 메소드는 가려지고
		 	자식 클래스에서 오버라이딩한 showState() 메소드가 호출된다. 
		*/
