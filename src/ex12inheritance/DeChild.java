...
public class DeChild extends DeParent {
	...
	메소드 오버라이딩의 조건
		- 부모클래스에서 선언될 때와 비교해서
		  접근범위가 동일하거나 그보다 넓으면 가능하다.
		- 부모에서 default로 선언하면
		  자식에서는 default 혹은 그보다 넓은범위의 접근지정자를 사용하면 
		  오버라이딩이 가능하다.
	 */
	@Override
	public String sleep() {
	...
	클래스 외부(메소드영역)에 로드되므로 오버라이딩되지 않고
	호출할 때 각 클래스명을 통해 호출해야 한다.
	 */
