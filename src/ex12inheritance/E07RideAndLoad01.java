...
-오버로딩
 : 메소드명은 같으나 매개변수의 개수, 타입, 순서가 다른 경우를 말한다.
	;서로 다른 메소드이므로 상속받은 하위클래스에서는
	오버로딩 된 모든 메소드가 포함된다.
-오버라이딩
 : 메소드명, 매개변수, 반환형까지 모두 동일한 메소드를 상속관계에 있는
 	하위클래스에서 재정의 한것을 말한다.
...
class C extends B {
	...
	상위클래스에 정의된 반환형, 메소드명, 매개변수의 개수가 모두 동일하다.
	 */
	@Override
	public void rideMethod() {
	...
	상위클래스에 정의된 메소드와 매개변수의 개수, 타입이 서로 다르다.
	 */
	public void loadMethod(double num) {
		System.out.println("C의 loadMethod");
	}
	/*
	따라서 C클래스로 객체를 생성하면 오버라이딩된 메소드는 모두 가려지게되므로
	C클래스에 정의된 메소드 하나만 남게된다.
	오버로딩된 메소드는 서로 다르므로 3개모두 존재하게된다.
	 */
}
...
//		ref1.loadMethod(1.1);	->C클래스 영역은 접근불가
		...
		/*
		참조변수 refId1이 C객체를 참조하고 있으므로
		마치 refId1이 C타입이라고 착각할수 있으나,
		처음부터 refId1은 A타입의 참조변수이다.
		자식이 부모를 참조하는것은 불가능하므로
		아래 2개의 문장은 에러가 발생한다.
		 */
		A refId1 = new C();		//가능
