...
//	public Computer() {}
	/*
	생성자를 개발자가 만들면 디폴트 생성자는 생성되지 않는다.
	 */
	public Computer(String name) {
		owner = name;
	}
	/*
	private 접근지정자로 선언된 멤버메소드는
	외부클래스 혹은 상속받은 클래스에서 접근이 불가능하므로
	public으로 선언된 메소드를 통해 간접 호출해야 한다.
	상속관계라면 protected로 선언된 메소드로 호출할 수도 있다.
	 */
...
class NotebookComputer extends Computer {
	...
	앞에 다른 문장이 먼저오면 오류가 발생하므로 주의해야 한다.
	여기서는 부모의 인자생성자를 호출하고있다.
	 */
	public NotebookComputer (String name, int initCharge) {
		/*
		여기서 아래의 super()문장을 삭제하면 에러가 발생한다.
		부모클래스에서 인자생성자를 정의하였으므로
		디폴트 생성자는 만들어지지 않았기 때문이다.
		 */
		super(name);
		battary = initCharge;
	}
	...
	public void movigCal() {
		...
		//private으로 선언된 멤버메소드이므로 직접 호출할 수 없다.
//		keyboardTouch();	->에러발생.
//		calculate();	->에러발생.
		calculateFunc();
...
/*
태블릿 노트북
 : 컴퓨터의 기본인 계산기능, 노트북의 휴대이용 기능,
 여기에 펜으로 드로잉 할수있는 기능까지 추가된 컴퓨터이다.
 */
